...
protected:
  ...
  inline void _print_progress() {
    ...
    if (has_total_it || print_bar) {
      ...
      compute_pbar_size(pbar_pct_str.length() + pbar_suf_str.length() +
                        suffix.length() + 2);
  ...
  inline void _internal_update_end() { suffix_.str(""); }
  ...
  inline bool _internal_update() {
    ...
    if (is_tty && (cur_ % period == 0 || about_to_finish)) {
      ...
      if (nupdates > 10) {
        period = (int)(std::min(std::max((1.0 / 25) * cur_ / __tmp_dt_tot, 1.0),
                                5e5));
        smoothing = 25 * 3;
      }
      _compute_total();
...
public:
  ...
  template <class T> pm &operator<<(const T &t);
...
template <class It> class IteratorProgressMonitor : public pm {
protected:
  // update() should not be called directly on iterator wrapper
  void update() { pm::update(); }
...
public:
  ...
  public:
    ...
    inline reference operator*() const { return *iter_; }
...
template <class IntType> class RangeContainer {
  class RangeIterator {
  public:
    ...
    IntType const &operator*() const { return value; }
...
public:
  ...
  RangeIterator begin() const { return RangeIterator(from, step); }

  RangeIterator end() const { return RangeIterator(to, -step); }
